...
import com.thr.mobilemedical.utils.ABViewUtil;
...
public class WheelView extends ScrollView {
	...
	private TextView createView(String item) {
		...
		int padding = DensityUtils.dp2px(context, 7);
		tv.setPadding(padding, padding, padding, padding);
		if (0 == itemHeight) {
			itemHeight = ABViewUtil.getViewMeasuredHeight(tv);
			L.d(TAG, "itemHeight: " + itemHeight);
	...
	public void setBackgroundDrawable(Drawable background) {
		...
		if (viewWidth == 0) {
			...
			L.d(TAG, "viewWidth: " + viewWidth);
		}

		if (null == paint) {
			paint = new Paint();
			// 设置上下边框的颜色
			paint.setColor(getResources().getColor(R.color.other_user));
			paint.setStrokeWidth(DensityUtils.dp2px(context, 1f));
		}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		L.d(TAG, "w: " + w + ", h: " + h + ", oldw: " + oldw + ", oldh: "
				+ oldh);
		viewWidth = w;

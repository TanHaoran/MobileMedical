...
public class HandleActivity extends Activity {
		...
		if ("\"1\"".equals(result)) {
			...
			if (state == LONG) {
				loadHandlerOrder(LoginInfo.patient.getPATIENTHOSID(),
						...
						Method.ORDER_LONG, Method.HANDLE);
			} else {
				loadHandlerOrder(LoginInfo.patient.getPATIENTHOSID(),
						LoginInfo.patient.getPATIENTINTIMES(),
						Method.ORDER_TEMP, Method.HANDLE);
			}
		} else {
			content = "提交失败";
		}
	...
	private void loadHandlerOrder(String patientInHosId, String patientInTimes,
		...
		httpGet.doGet(url, mDialog, this, "处置长期医嘱");
	}
	 ...
	 * @param orderList
	 */
	private void initOrderList(final List<Order> orderList) {
		mAdapter = new OrderAdapter(this, orderList, R.layout.item_order);
		if (orderList != null && orderList.size() > 0
			...
			mListView.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					if (orderList.get(position).getEXECTIME() == null
							|| "".equals(orderList.get(position).getEXECTIME())) {
						TextView textView = new TextView(HandleActivity.this);
						...
						MyDialog myDialo = new MyDialog(HandleActivity.this,
								textView, new SureClickListener() {

									@Override
									public void onSureClick() {
										postDropOrder(orderList.get(position));
									}
								});
						myDialo.show();
